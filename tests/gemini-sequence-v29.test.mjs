...
test("la sÃ©quence explicite garde une seule rÃ©ponse finale", () => {
...
test("Gemini est tentÃ© au maximum deux fois avec une double vÃ©rification courte", () => {
  ...
  assert.match(page, /Attends voir, matelotâ€¦ jâ€™aime pas trancher quand jâ€™ai un doute\./);
  assert.match(helper, /timeoutMs = TIMEOUT_MS/);
});

test("le mode hors ligne ne lance aucune requÃªte et le cache publie le nouveau bundle", () => {
  assert.match(page, /if\(!t\.online\)\{await new Promise/);
  assert.match(page, /i\(t\.local\.dialogue,!1\);return/);
  assert.match(worker, /bosco-pwa-v28-gemini-3\.5/);
